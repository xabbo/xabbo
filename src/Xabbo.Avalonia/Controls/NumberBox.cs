...
public partial class NumberBox : TemplatedControl
{
    ...
    {
        if (_textBox is { Text.Length: int length })
        {
            _textBox.SelectionStart = _textBox.SelectionEnd = _textBox.CaretIndex = length;
        }
    }
    ...
    private double CoerceValueToRange(double val)
    {
        ...
        if (ValidationMode is NumberBoxValidationMode.InvalidInputOverwritten)
        {
            if (double.IsNaN(val))
            {
                val = Value;
